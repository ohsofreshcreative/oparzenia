...
<section data-gsap-anim="section" @if(!empty($section_id)) id="{{ $section_id }}" @endif class="b-experts -smt {{ $sectionClass }} {{ $section_class }}">
	...
	<div class="c-main grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 items-center">
		@if (!empty($gallery))
		@foreach($gallery as $image)
		<div>
			{!! wp_get_attachment_image($image['ID'], 'full', false, ['class' => 'w-full h-full max-h-[600px] object-top object-cover']) !!}
		</div>
		@endforeach
		@endif
